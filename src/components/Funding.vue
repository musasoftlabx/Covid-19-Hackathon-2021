...
export default {
  ...
  methods: {
    Metrics(date) {
      ...
      date;
      let cleaned = fundings.map((obj, i) => ({
        category: obj.categories[0],
        start: obj.date + "00:00",
        end: obj.date + "23:59",
        color: colorSet.getIndex(i),
        donor: obj.donor,
        text: "Ksh " + obj.amount.toLocaleString(),
        textDisabled: true,
        icon:
          obj.categories[0] === "Cash"
            ? `${this.$store.getters.server}img/cash.png`
            : obj.categories[0] === "PPEs"
            ? `${this.$store.getters.server}img/ppe.png`
            : obj.categories[0] === "Service"
    ...
    Timeline(data) {
      ...
      chart.curveContainer.padding(140, 200, 50, 0);
